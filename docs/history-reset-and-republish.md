...
```bash
for crate in use-arithmetic use-number use-integer use-rational use-interval use-real use-complex use-geometry use-combinatorics use-series use-catalan use-geode use-algebra use-vector use-matrix use-linear use-calculus use-probability use-statistics use-trigonometry use-logic use-set; do
	cargo publish -p "$crate"
done
```
